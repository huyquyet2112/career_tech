$(document).ready(function () {
    $('#adminRecruiterTable').DataTable({
        ...
        language: {
            emptyTable: stringAdminRecruiter.NoRecruiters,
            paginate: {
                previous: "&laquo;",
                next: "&raquo;",
            }
        ...
        columns: [
            {
                ...
                render: function (data, type, row) {
                    if (row.RecuiterAvatar != null) {
                        return `
                            <div class="d-flex justify-content-center align-items-center">
                                <a href="recruiters/${row.UserId}">
                                    <img src="${row.RecuiterAvatar}" class="avatar-lg2 border shadow-sm" />
                                </a>                        
                            </div>
                        `
                    } else {
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `
                            <p>${stringAdminRecruiter[row.Status]}</p>
                        `
                }
            },
            {
                ...
                render: function (data, type, row) {
                    return `
                            <p>${stringAdminRecruiter[row.VerifyStatus]}</p>
                        `
                }
            },
            {
                    ...
                    return `
                        <div class="d-flex justify-content-center align-items-center">
                            <button type="button" class="btn btn-info btn-sm custom-icon-action-datatable me-1 mr-1"
                                onclick="window.location.href='/admin/recruiters/${row.UserId}'">
                                <i class="fas fa-info-circle"></i>
                            </button>
                            <button type="button" class="btn btn-secondary border btn-sm custom-icon-action-datatable"
                                onclick="editStatusRecruiter(this)" data-userid="${row.UserId}">
    ...
    function getUrlAdminRecruiters() {
        const baseUrl = '/api/admin/recruiters';
    ...
    window.editStatusRecruiter = async function (button) {
        $('#formEditStatusRecruiter')[0].reset();
        ...
        $('#statusRecruiter > option').each(function (index, option) {
            if (option.value == data.Status) {
                $(option).prop('selected', true);
                return false;
            }
        });
        $('#statusRecruiter').selectpicker('destroy');
        $('#statusRecruiter').selectpicker();


        $('#verifyStatusRecruiter > option').each(function (index, option) {
            if (option.value == data.VerifyStatus) {
                $(option).prop('selected', true);
                return false;
            }
        });
        $('#verifyStatusRecruiter').selectpicker('destroy');
        $('#verifyStatusRecruiter').selectpicker();

        $('#editStatusRecruiterModal').modal('show');
        $('#formEditStatusRecruiter').attr('data-userid', userId);
    }

    $('#formEditStatusRecruiter').submit(function (e) {
        $('.error-message').text('');
        e.preventDefault();
        const userId = $(this).attr('data-userid');
        const status = $(this).find('#statusRecruiter').val();
        const verifyStatus = $(this).find('#verifyStatusRecruiter').val();
        ...
        $.ajax({
            ...
            success: function (dataReturn) {
                changeStatusModal('editStatusRecruiterModal', 'close');
                showToast(dataReturn.Message, 'text-white bg-success');
                $('#adminRecruiterTable').DataTable().ajax.reload();
            },
            error: function (xhr) {
